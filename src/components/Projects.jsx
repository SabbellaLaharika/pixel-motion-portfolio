  ...
  return (
    <section id="projects" className="py-24 bg-[#f8fafc] dark:bg-black transition-colors overflow-hidden">
      <div className="container mx-auto px-6 max-w-7xl">
          ...
          {projects.map((project, index) => {
            ...
            return (
              <motion.div
                ...
                className="snap-start shrink-0 cursor-pointer py-4"
                style={{ width: "min(380px, 90vw)" }}
                onClick={() => handleCardClick(index)}
              >
                <div
                  className={`group relative h-full bg-white dark:bg-gray-900/40 border rounded-[2rem] flex flex-col overflow-hidden transition-all duration-500 ${
                    isSelected
                      ? "border-primary shadow-2xl shadow-primary/20 ring-4 ring-primary/10"
                      : "border-black/5 dark:border-white/5 hover:shadow-2xl hover:border-primary/40 hover:-translate-y-2"
                  }`}
                >
                  <div className="absolute top-4 right-4 z-20">
                    <span className="px-3 py-1 bg-white/90 dark:bg-black/50 backdrop-blur-md text-[10px] font-black uppercase tracking-widest text-primary border border-primary/20 rounded-full shadow-sm shadow-primary/10">
                      {project.category}
                    </span>
                  </div>
                  ...
                  <div className="mx-7 relative aspect-[1.8/1] overflow-hidden rounded-2xl bg-gray-50 dark:bg-black/40 border border-black/[0.08] dark:border-white/[0.08] shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
                    <img
                      src={project.image}
                      alt={project.title}
                      style={{ objectPosition: project.objectPosition || "center" }}
                      className="w-full h-full object-cover scale-[1.15] transition-transform duration-700 group-hover:scale-[1.2] opacity-90 group-hover:opacity-100"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/[0.03] dark:from-black/[0.1] to-transparent pointer-events-none" />
                  ...
                  <div className="p-7 pt-6 mt-auto">
                    <div className="flex flex-wrap items-center gap-1.5 mb-6">
                      {visibleTech.map((t) => (
                        <span
                          key={t}
                          className="px-2.5 py-1 bg-gray-100 dark:bg-white/5 text-[9px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-tighter rounded-md border border-black/5 dark:border-white/5"
                        >
                          {t}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center justify-between border-t border-black/5 dark:border-white/5 pt-5">
                      ...
                      {project.demo && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-[10px] font-black uppercase tracking-[0.2em] text-primary flex items-center group/demo"
                          onClick={(e) => e.stopPropagation()}
                        >
        ...
        {/* Detail Panel — Refined Engineering Case Study Modal */}
        <AnimatePresence mode="wait">
          {selectedProject && (
            <motion.div
              ...
              className="mt-12"
            >
              <div className="bg-white dark:bg-gray-900/60 border border-black/[0.05] dark:border-white/10 rounded-[3rem] p-6 md:p-12 shadow-3xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full blur-3xl -z-10 translate-x-1/2 -translate-y-1/2" />
                
                {/* Header Section */}
                <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12 pb-10 border-b border-black/5 dark:border-white/5">
                  <div className="max-w-3xl">
                    <div className="flex items-center gap-3 mb-5">
                      ...
                      <span className="text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">
                        {selectedProject.tech[0]} Infrastructure
                      </span>
                    </div>
                    <h4 className="text-3xl md:text-6xl font-extrabold text-gray-900 dark:text-white mb-6 tracking-tight">
                      {selectedProject.title}
                    </h4>
                    <p className="text-lg md:text-xl text-gray-600 dark:text-gray-400 font-semibold leading-relaxed max-w-2xl">
                      {selectedProject.description}
                    </p>
                  </div>
                  <button
                    onClick={() => setSelectedIndex(null)}
                    className="md:absolute top-8 right-8 w-14 h-14 rounded-full bg-gray-100 dark:bg-white/5 hover:bg-red-500 hover:text-white flex items-center justify-center transition-all border border-black/5 dark:border-white/10 group shadow-lg"
                  >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
                  {/* Left Column: Architecture Visuals */}
                  <div className="space-y-10">
                    <motion.div
                      initial={{ opacity: 0, scale: 0.98 }}
                      animate={{ opacity: 1, scale: 1 }}
                      className="relative rounded-[2rem] overflow-hidden border border-black/5 dark:border-white/10 shadow-2xl cursor-zoom-in group bg-gray-50 dark:bg-black/20"
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all flex items-center justify-center">
                        <div className="bg-white/95 dark:bg-black/90 p-5 rounded-2xl opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 shadow-2xl flex items-center gap-3 border border-primary/20">
                          <ZoomIn size={20} className="text-primary" />
                          <span className="text-xs font-black uppercase tracking-widest">Open Full Diagram</span>
                        </div>
                      </div>
                    </motion.div>

                    {/* Engineering Highlights - Compact & Denser */}
                    <div className="bg-gray-50 dark:bg-white/5 rounded-[2rem] p-8 border border-black/[0.05] dark:border-white/[0.05] shadow-inner">
                      <div className="flex items-center gap-3 mb-6">
                        <Zap size={18} className="text-primary fill-primary/20" />
                        <h5 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500">Key Engineering Highlights</h5>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        {selectedProject.engineeringHighlights.map((highlight, i) => (
                          <div key={highlight} className="flex items-center gap-3 px-4 py-3 bg-white dark:bg-gray-900/50 rounded-xl border border-black/[0.03] dark:border-white/[0.03] shadow-sm">
                            <div className="w-1.5 h-1.5 rounded-full bg-primary/40" />
                            <span className="text-[11px] font-bold text-gray-700 dark:text-gray-300 tracking-tight">{highlight}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Right Column: Case Study Bullets */}
                  <div className="flex flex-col gap-12">
                    <div className="space-y-12">
                      <section className="relative">
                        <h5 className="text-[10px] font-black text-red-500 dark:text-red-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                          <AlertTriangle size={14} /> The Problem
                        </h5>
                        <ul className="space-y-4 max-w-xl">
                          {selectedProject.problem.map((bullet, i) => (
                            <li key={i} className="flex items-start gap-4 text-sm md:text-base text-gray-600 dark:text-gray-400 leading-relaxed font-semibold">
                      ...
                      <div className="h-px bg-black/[0.05] dark:bg-white/[0.05] w-24" />
                      ...
                      <section className="p-8 bg-primary/[0.03] dark:bg-primary/[0.05] rounded-[2rem] border border-primary/10 shadow-sm relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                          <LineChart size={80} className="text-primary" />
                        </div>
                        <h5 className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-8 flex items-center gap-3">
                        ...
                        <div className="space-y-6">
                          {selectedProject.highlight.map((metric, i) => (
                            <div key={i} className="relative pl-6 border-l-2 border-primary/20">
                              <p className="text-xl md:text-2xl text-gray-900 dark:text-white font-black tracking-tight leading-none mb-1">
                                {metric}
                              </p>
                              {i === 0 && <span className="text-[10px] font-bold text-primary/60 uppercase">Primary Engineering Win</span>}
                            </div>
                          ))}
                        </div>
                      </section>
                    ...
                    <div className="pt-4">
                      <h5 className="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                        <Layers size={14} /> Infrastructure Stack
                      </h5>
                      <div className="flex flex-wrap gap-2.5">
                        {selectedProject.tech.map((t) => (
                          <span 
                            key={t} 
                            className="px-4 py-2.5 bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-300 rounded-xl text-[11px] font-black uppercase tracking-widest border border-black/[0.05] dark:border-white/[0.05] shadow-sm"
                          >
                            {t}
                          </span>
                        ))}
                    ...
                    <div className="flex flex-wrap gap-4 pt-10">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 min-w-[180px] flex items-center justify-center gap-3 px-8 py-5 bg-gray-900 dark:bg-white text-white dark:text-black rounded-2xl text-xs font-black uppercase tracking-[0.2em] hover:shadow-2xl transition-all border border-transparent dark:border-white/10"
                      >
                      ...
                      {selectedProject.demo && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 min-w-[180px] flex items-center justify-center gap-3 px-8 py-5 bg-primary text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] hover:shadow-2xl transition-all shadow-xl shadow-primary/20"
                        >
                          Production Link <ExternalLink size={18} />
                        </a>
                      )}
                    </div>
                  </div>
        ...
        {zoomedImage && (
          ...
          >
            ...
            >
              ...
              />
              <button
                className="absolute top-4 right-4 bg-black/60 hover:bg-black/80 text-white rounded-full p-5 backdrop-blur-md transition-all border border-white/10 flex items-center justify-center"
